...
def update_leaderboard(file_name, leader_names, leader_scores, player_name,
                       player_score):

  index = 0
  # TODO 8: loop through all the scores in the existing leaderboard list

  for each in range(0, len(scores)):
  ...
  for leader_index in range(len(scores)):
    leaderboard_file.write(names[leader_index] + "," +
                           str(scores[leader_index]) + "\n")
...
def draw_leaderboard(high_scorer, leader_names, leader_scores, turtle_object,
  ...
  for index in range(len(names)):
    turtle_object.write(str(index + 1) + "\t" + names[index] + "\t" +
                        str(scores[index]),
                        font=font_setup)
    turtle_object.penup()
  ...
  if player_score >= scores[len(scores) - 1]:
  ...
  elif player_score < gold_score and player_score >= silver_score:
